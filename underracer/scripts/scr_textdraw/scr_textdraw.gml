function scr_textdraw() {
	...
	if shake>38
	{
	  if shake=39 //earthquake until end
	     ...
	     draw_text_transformed(myx+hspeed, myy+vspeed, string_hash_to_newline(myletter),1.5,1.5,0) 
	     }
	  if shake=40 
	  draw_text(myx+hspeed, myy+vspeed, string_hash_to_newline(myletter)); //all roll
	  ...
	  if shake=42 //drifting text
	    ...
	    if shake=43 //singing
	     ...
	     draw_text_transformed(myx+hspeed*0.70+10, myy+vspeed*0.70, string_hash_to_newline(myletter),1.5,1.5,0) 
	     direction = direction-30*n;
	     }
	}
	else 
